...
public class IntelligentHotWords {
    ...
    /** 保存所有的热词 */
    private static Map<String,String> totalHotWordMap;
  static {
      totalHotWordMap=new LinkedHashMap<>();
            mergeUserWordAndCourWord();
       }

    private static IntelligentHotWords instance=new IntelligentHotWords();
    public static IntelligentHotWords getInstance(){return instance;}
    private IntelligentHotWords(){
        recodeHotWord();
     ...
     *   用户热词放在靠后，提高权重
     */
    private static void mergeUserWordAndCourWord(){
        totalHotWordMap.putAll(SentenceCode.getInstance().getCourWordTable());
        totalHotWordMap.putAll(SentenceCode.getInstance().getUserWordTable());
    }
    ...
    public String nearToneCorrection(String sentence){
         ...
         String sentenceCode=SentenceCode.getInstance().sentenceConvertToNormalCode(sentence);
         ...
         for(Map.Entry<String,String> entry:sentenceItem.entrySet()){
             ...
             while (handledCount!=matchWordCount){
                 ...
                 if (1==handledCount){
                     ...
                     String notHandleSentenceCode=SentenceCode.getInstance().sentenceConvertToNormalCode(notHandleSentence);
    ...
    private  String getHotWordBeginPunctuation(String hotWord){
        ...
        for (char item : hotWord.toCharArray()){
            if(Tools.isChineseChar(item)){ break; }
            resultPun+=item;
        }
        return resultPun;
    }
    ...
    */
     ...
     *
     */
     public void recodeHotWord()
     {
         SentenceCode.getInstance().reloadFuzzySetting(Config.getFuzzyOption());
     ...
     public void reloadUserWord(){
         SentenceCode.getInstance().loadLocalHotWord();
         totalHotWordMap.clear();
         mergeUserWordAndCourWord();
     }
